...
/**
 * Single-file version of Focus Session Tracker
 * Contains all functionality in one file for easy compilation
 */
public class FocusTracker {
    ...
    private static void runApplication() {
        System.out.println("=== FOCUS SESSION TRACKER ===");
        ...
        try {
            ...
            while (rs.next()) {
                ...
                System.out.printf("ID: %d | Task: %s | Duration: %ds | Energy: %d/5\n", 
                    rs.getInt("id"), rs.getString("task_description"), 
                    rs.getInt("duration"), rs.getInt("energy_level"));
    ...
    private static void showStatistics() {
        try {
            ResultSet rs = connection.createStatement().executeQuery(
                "SELECT COUNT(*) as count, AVG(duration) as avg_duration, AVG(energy_level) as avg_energy FROM session_logs");
            
            if (rs.next()) {
                System.out.println("\n=== STATISTICS ===");
                System.out.printf("Total Sessions: %d\n", rs.getInt("count"));
                System.out.printf("Average Duration: %.1f minutes\n", rs.getDouble("avg_duration") / 60);
                System.out.printf("Average Energy: %.1f/5\n", rs.getDouble("avg_energy"));
            }
